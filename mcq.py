...
import json
import random
from dynamic_db_handlercop1 import dynamic_db_handler
import os
...
def get_mcq_db_connection(subject=None):
    """Get connection to appropriate MCQ database using persistent disk"""
    if subject:
        ...
        for db_info in mcq_databases:
            ...
            # Ensure path uses persistent disk
            if not os.path.isabs(db_file):
                db_file = os.path.join(PERSISTENT_DISK_PATH, os.path.basename(db_file))
...
def get_user_db_connection():
    """Get centralized user database connection using persistent disk"""
...
def create_default_mcq_database():
    """Create a default MCQ database if none exists in persistent storage"""
    db_path = os.path.join(PERSISTENT_DISK_PATH, 'general_mcq.db')
    ...
    conn.commit()
    print(f"✅ Created default MCQ database at: {db_path}")
    return conn
...
def debug_mcq_database_schema():
    """Debug MCQ database schema and fix missing columns using persistent disk"""
    try:
        ...
        debug_output.append(f"📁 Persistent Disk Path: {PERSISTENT_DISK_PATH}")
        debug_output.append(f"🔗 Using Dynamic DB Handler: {type(dynamic_db_handler).__name__}")
        ...
        if not tables:
            ...
            debug_output.append("✅ MCQ database tables created successfully in persistent storage")
            return "\n".join(debug_output)
...
def fix_mcq_schema_immediately():
    """Quick fix for missing difficulty_filter column using persistent disk"""
    try:
        conn = get_mcq_db_connection()
...
def fix_mcq_questions_schema():
    """Fix missing columns in mcq_questions table using persistent disk"""
    try:
        conn = get_mcq_db_connection()
...
def mcq_home():
    ...
    try:
        ...
        fix_mcq_questions_schema()      # For questions table
        print(f"📁 MCQ module using persistent disk path: {PERSISTENT_DISK_PATH}")
    except Exception as e:
